"""VENDORED COPY of molab_bridge/remote_detector.py -- see protocol.py in
this same directory for why this is a copy rather than a shared import, and
port any change to both.

The only edit from the original: `from protocol import ...` below is now
`from .protocol import ...`, since this copy lives in a package
(features/cloud/) instead of being run with molab_bridge/ on sys.path.
Everything else, including the module docstring below, is unmodified.
...
NOTHING IN features/ IMPORTS THIS DIRECTLY. features/cloud/ is the seam --
see its README. The local models stay exactly as they are and stay default.
"""
import time
